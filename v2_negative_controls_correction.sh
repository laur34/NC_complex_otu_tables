...
# find maximum of columns with NEG_CTRL_PATTERN in header
tail -n +2 $OTU_TABLE | \
	cut -f $(head -1 $OTU_TABLE | \
	tr "\t" "\n" | \
	grep -n $NEG_CTRL_PATTERN | \
	cut -d":" -f1 | \
	tr "\n" "," | \
	perl -pe 's/,$/\n/g') | \
...
# find columns without NEG_CTRL_PATTERN in header ...
# ...and join with the maximum negative control column
tail -n +2 $OTU_TABLE | \
	cut -f $(head -1 $OTU_TABLE | \
	tr "\t" "\n" | \
	grep -n "" | \
	grep -v $NEG_CTRL_PATTERN | \
	cut -d":" -f1 | \
	tr "\n" "," | \
	perl -pe 's/,$/\n/g') \
...
cat biol_samples_header.tsv \
	<(paste -d $'\t' <(tail -n +2 $OTU_TABLE | cut -f1) filtered_biol_samples_nohead.tsv) \
	> filtered_${OTU_TABLE}
